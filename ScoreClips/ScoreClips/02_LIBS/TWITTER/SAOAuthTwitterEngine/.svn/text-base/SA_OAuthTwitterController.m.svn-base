...
- (void) loadView {
	...
	if ( UIInterfaceOrientationIsLandscape( self.orientation ) ) {
		self.view = [[[UIView alloc] initWithFrame: CGRectMake(0, 0, 480, 300)] autorelease];	
		_backgroundView.frame =  CGRectMake(0, 44, 480, 268);
...
//	UINavigationItem				*navItem = [[[UINavigationItem alloc] initWithTitle: NSLocalizedString(@"Twitter Info", nil)] autorelease];
    UINavigationItem				*navItem = [[[UINavigationItem alloc] initWithTitle: NSLocalizedString(@"Connect with Twitter", nil)] autorelease];
    // MinhPB 2012/03/15
...
//    navItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithTitle:@"Cancel" style:UIBarButtonItemStyleBordered target:self action:@selector(cancel:)] autorelease];
...
//    NSLog(@"webViewDidFinishLoad: %@", webView.request.URL.absoluteString);
    ...
    // MinhPB 2012/10/30, resolve the invalid value
//    if ([webView.request.URL.absoluteString isEqualToString:@"https://twitter.com/home"]) {
//        [webView stopLoading];
//        [webView goBack];
//        [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:_engine.authorizeURL.absoluteString]]];
//        return;
//    }
...
//    NSLog(@"shouldStartLoadWithRequest: %@", webView.request.URL.absoluteString);
//    if ([webView.request.URL.absoluteString isEqualToString:@"https://twitter.com/home"]) {
//        [webView stopLoading];
//        [webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:_engine.authorizeURL.absoluteString]]];
//        return YES;
