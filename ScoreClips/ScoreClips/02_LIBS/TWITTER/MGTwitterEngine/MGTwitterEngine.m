...
- (NSString *)sendUpdate:(NSString *)status withLocation:(NSString *) coordinateString
{
    return [self sendUpdate:status inReplyTo:0 withLocation:coordinateString];
}

- (NSString *)sendUpdate:(NSString *)status inReplyTo:(unsigned long)updateID withLocation:(NSString *) coordinateString;
{
    if (!status) {
        return nil;
    }
    ...
    //dongha 2013_04_16
    if (coordinateString != nil & coordinateString.length > 0) {
        NSArray *ar = [coordinateString componentsSeparatedByString:@","];
        if (ar.count == 2) {
            [params setObject:[ar objectAtIndex:0] forKey:@"lat"];
            [params setObject:[ar objectAtIndex:1] forKey:@"long"];
        }
    }
    //end
    if (updateID > 0) {
