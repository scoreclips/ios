...
- (MGTwitterEngine *)initWithDelegate:(id)newDelegate
{
    if (self = [super init]) {
...
//    NSString *path = [NSString stringWithFormat:@"statuses/update.%@", API_FORMAT];
    NSString *path = [NSString stringWithFormat:@"statuses/update.%@", @"json"];// MinhPB 2012/02/23
...
//ThinhHQ 2012/2/22 tweet with location

- (NSString *)sendUpdate:(NSString *)status andLat:(NSString*)latTitue andLong: ( NSString*)longTitue
{
    if (!status) {
        return nil;
    }
    ...
    NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:0];
    [params setObject:trimmedText forKey:@"status"];
    [params setObject:latTitue forKey:@"lat"];
    [params setObject:longTitue forKey:@"long"];
    NSString *body = [self _queryStringWithBase:nil parameters:params prefixed:NO];
    NSLog(@"BODY=%@",body);
    return [self _sendRequestWithMethod:HTTP_POST_METHOD path:path 
                        queryParameters:params body:body 
...
- (NSString *)uploadImage:(NSString *)image status:(NSString *)status
{
	//NSLog(@"image = %@", image);
	//NSLog(@"status = %@", status);
	...
	NSString *path = @"http://upload.twitter.com/1/statuses/update_with_media.json";
	...
	NSMutableDictionary *params = [NSMutableDictionary dictionaryWithCapacity:0];
	[params setObject:trimmedText forKey:@"status"];
	[params setObject:image forKey:@"media[]"];
	...
	OAConsumer *_consumer = [[OAConsumer alloc] initWithKey:OAUTH_CONSUMER_KEY secret:OAUTH_CONSUMER_SECRET];
	OAToken *accessToken = [[OAToken alloc] initWithKey:OAUTH_ACCESS_TOKEN secret:OAUTH_ACCESS_TOKEN_SECRET];
	
	OAMutableURLRequest *theRequest = [[[OAMutableURLRequest alloc] initWithURL:finalURL consumer:_consumer token:accessToken realm:nil signatureProvider:nil] autorelease];
	...
	[body appendData:[[NSString stringWithString:@"\r\n"] dataUsingEncoding:NSUTF8StringEncoding]];
	[body appendData:[[NSString stringWithString:status] dataUsingEncoding:NSUTF8StringEncoding]];
	[body appendData:[[NSString stringWithFormat:@"\r\n--%@--\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    // Create a connection using this request, with the default timeout and caching policy, 
    // and appropriate Twitter request and response types for parsing and error reporting.
	MGTwitterRequestType requestType = MGTwitterImageRequest;
	MGTwitterResponseType responseType = MGTwitterImage;
	...
	if (!connection)
	{
		NSLog(@"can not upload image to Twitter");
		return nil;
	}
	else
	{
