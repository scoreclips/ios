...
- (void)requestTokenFromProvider
{
    LI_OAMutableURLRequest *request = 
            [[[LI_OAMutableURLRequest alloc] initWithURL:requestTokenURL
                                             consumer:self.consumer
                                                token:nil   
                                             callback:linkedInCallbackURL
                                    signatureProvider:nil] autorelease];
    //[[[LI_OAMutableURLRequest alloc] initWithURL:requestTokenURL consumer:self.consumer token:nil realm:linkedInCallbackURL signatureProvider:nil] autorelease];
    
    
    [request setHTTPMethod:@"POST"];   
    
    OARequestParameter *nameParam = [[OARequestParameter alloc] initWithName:@"scope"
                                                                       value:@"r_fullprofile+r_emailaddress"];
    NSArray *params = [NSArray arrayWithObjects:nameParam, nil];
    [request setParameters:params];
    OARequestParameter * scopeParameter=[OARequestParameter requestParameterWithName:@"scope" value:@"r_fullprofile r_emailaddress"];
...
- (void)requestTokenResult:(OAServiceTicket *)ticket didFinish:(NSData *)data 
{
    ...
    self.requestToken = [[LI_OAToken alloc] initWithHTTPResponseBody:responseBody];
...
{ 
    LI_OAMutableURLRequest *request = 
            [[[LI_OAMutableURLRequest alloc] initWithURL:accessTokenURL
                                             consumer:self.consumer
                                                token:self.requestToken   
                                             callback:nil
                                    signatureProvider:nil] autorelease];

    //[[[LI_OAMutableURLRequest alloc] initWithURL:accessTokenURL consumer:self.consumer token:self.requestToken realm:nil signatureProvider:nil] autorelease];
...
- (void)accessTokenResult:(OAServiceTicket *)ticket didFinish:(NSData *)data 
{
    ...
    {
        self.accessToken = [[LI_OAToken alloc] initWithHTTPResponseBody:responseBody];
    }
    // Notify parent and close this view
    [[NSNotificationCenter defaultCenter] 
     postNotificationName:@"loginViewDidFinish"        
